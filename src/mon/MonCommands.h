...
COMMAND("pg dump_stuck type=CephChoices,strings=inactive|unclean|stale,name=stuckops,req=false type=CephInt,name=threshold,req=false", "show information about stuck pgs [--threshold=seconds to consider stuck]")
...
COMMAND("osd crush add-bucket type=CephString,name=name type=CephString,name=type", "add no-parent (probably root) crush bucket <name> of type <type>")
COMMAND("osd crush set type=CephInt,name=id,range=0 type=CephName,name=name,req=false type=CephFloat,name=weight,range=0.0 type=CephString,name=args,n=N", "set crushmap entry for <id> to <weight> with location <args>")
COMMAND("osd crush add type=CephInt,name=id,range=0 type=CephName,name=name,req=false type=CephFloat,name=weight,range=0.0 type=CephString,name=args,n=N", "add crushmap entry for <id> with <weight> and location <args>")
...
COMMAND("osd crush link type=CephString,name=name type=CephString,name=args,n=N", "link existing entry for <name> under location <args>")
...
COMMAND("osd crush unlink type=CephString,name=name type=CephString,name=ancestor,req=false", "unlink <name> from crush map (everywhere, or just at <ancestor>")
...
COMMAND("osd pool set type=CephPoolname,name=pool type=CephChoices,name=var,strings=size|min_size|crash_replay_interval|pg_num|pgp_num|crush_ruleset type=CephInt,name=val type=CephChoices,name=sure,strings=--allow-experimental-feature,req=false", "set pool parameter <var> to <val>")
COMMAND("osd reweight-by-utilization type=CephInt,name=oload,range=100,req=false", "reweight OSDs by utilization [overload-percentage-for-consideration, default 120]")
