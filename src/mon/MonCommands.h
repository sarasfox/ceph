...
COMMAND("mon tell type=CephString,name=who type=CephStrings,n=N,name=args", "send command to specific monitor(s)")
...
COMMAND("osd blacklist type=CephChoices,name=blacklistop,strings=add|rm type=CeplEntityAddr,name=addr type=CephFloat,name=expire,range=0.0,req=false", "add (optionally until <expire> seconds from now) or remove <addr> from blacklist")
